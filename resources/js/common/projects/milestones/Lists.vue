<template>
    <v-container grid-list-md>
        ...
        <v-layout row wrap>
            <span>{{ trans('data.schedule') }}</span>
            <v-spacer></v-spacer>
            <v-btn
                v-if="$can('project.' + projectId + '.milestone.create')"
                style="background-color:#06706d;color:white;" class="lighten-1" dark
                @click="create"
            >
                {{ trans('messages.add') }}
        ...
        <v-layout row wrap v-else>
            <v-flex xs12 sm12>
                <div>
                    <v-alert :value="true" color="teal" outline>
...
export default {
    ...
    created() {
        const self = this;
        self.projectId = self.projectId = self.$route.params.project_id;;
