    ...
    <div>
        <v-layout row justify-center>
            ...
            <v-dialog v-model="dialog" full-width>
                <v-card>
                    <v-card-title>
                        <div v-if="templateType == 'customer'">
                            <v-icon medium>person</v-icon>
                            <span class="headline">
                                {{ trans('messages.create_cutomer') }}
                            </span>
                        </div>
                        <div v-if="templateType == 'lead'">
                            <v-icon medium>verified_user</v-icon>
                            <span class="headline">
                                {{ trans('messages.create_lead') }}
                            </span>
                        </div>
                        <v-spacer></v-spacer>
                    ...
                    <v-divider></v-divider>
                    <v-form ref="customerFormAdd">
                        <v-container grid-list-md>
                            <v-flex xs12 sm12 md12 v-if="templateType == 'lead'">
                            ...
                            <v-flex xs12 sm12 md12>
                                <v-card class="elevation-3 mb-3">
                                    ...
                                    <v-card-text>
                                        ...
                                        <v-layout row v-if="templateType == 'lead'">
                                            <v-flex xs12 md12>
                                                <v-textarea
                            ...
                            <v-flex xs12 sm12 md12>
                                <v-card class="elevation-3 mb-3">
                                    <v-card-title>
                                        ...
                                        <span class="subheading">
                                            {{ trans('messages.primary_contact') }}
                                        </span>
                                    </v-card-title>
                                    <v-divider></v-divider>
                                    <v-card-text>
                                        <v-layout row>
                                            <v-flex xs12 sm4>
                                                <v-text-field
                                                    ...
                                                    :label="trans('messages.contact_name')"
                                                    v-validate="'required'"
                                                    data-vv-name="name"
                                                    :data-vv-as="trans('messages.contact_name')"
                                            ...
                                            <v-flex xs12 sm6 md4>
                                                <v-text-field
                                                    v-model="form_fields.contact_email"
                                                    :label="trans('messages.email')"
                                                    v-validate="'required|email'"
                                                    data-vv-name="form_fields.email"
                                                    :data-vv-as="trans('messages.email')"
                                                    :error-messages="
                                                        errors.collect('form_fields.email')
                                                    "
                                                    required
                                                ></v-text-field>
                                            </v-flex>
                                            <v-flex xs12 sm6 md4>
                                                <v-text-field
                                                    ...
                                                    :error-messages="
                                                        errors.collect('form_fields.password')
                                                    "
                                                    required
                                                ></v-text-field>
                                            </v-flex>
                                            <v-flex xs12 sm4 md4 v-if="templateType == 'lead'">
...
import StatusAdd from '../components/status/Add';
...
export default {
    ...
    methods: {
        create(data) {
            const self = this;
            self.templateType = data.template;
            self.contacted_date = null;
            if (data.template == 'lead') {
                self.url = '/admin/leads';
            } else if (data.template == 'customer') {
                self.url = '/admin/customers';
            }
        ...
        store() {
            ...
            if (self.templateType == 'lead') {
                data.status_id = self.form_fields.status_id;
                data.source_id = self.form_fields.source_id;
                data.assigned_to = self.form_fields.assigned_to;
                data.description = self.form_fields.description;
                    ...
                    axios
                        ...
                        .then(function(response) {
                            ...
                            if (response.data.success === true) {
                                self.dialog = false;
                                if (self.templateType == 'customer') {
                                    self.$eventBus.$emit('updateCustomerTable');
                                } else if (self.templateType == 'lead') {
