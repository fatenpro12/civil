...
export default {
    ...
    methods: {
        ...
        getReminder() {
            const self = this;
            axios
                .get('/admin/reminders/' + self.reminder_id + '/edit', {
                    params: { lead_id: self.lead_id },
                })
                .then(function(response) {
                    self.employees = response.data.employees;
                    self.form_fields = response.data.reminder;
                    self.remind_on = response.data.reminder.remind_on;
                    self.send_email = response.data.reminder.send_email;
                    self.dialog = true;
                })
        ...
        store() {
            ...
            self.$validator.validateAll().then(result => {
                if (result == true) {
                    self.loading = true;
                    axios
                        .put('/admin/reminders/' + self.reminder_id, data)
                        .then(function(response) {
                            self.loading = false;
