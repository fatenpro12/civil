        ...
        <v-dialog v-model="dialog" width="800">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="success" @click="store"> {{ trans('messages.update') }} </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    </v-layout>
...
export default {
    ...
    methods: {
        ...
        edit(data) {
            const self = this;
            self.invoiceSchemeId = data.id;
            self.$validator.reset();
            axios
                .get('/admin/invoice-scheme/' + data.id + '/edit')
                .then(function(response) {
                    self.name = response.data.name;
                    self.type = response.data.scheme_type;
                    self.prefix = response.data.prefix;
                    self.start_from = response.data.start_number;
                    self.is_default = response.data.is_default;
                    self.no_of_digits = response.data.total_digits;
                    self.preview();
                    self.dialog = true;
                })
        ...
        store() {
            ...
            self.$validator.validateAll().then(result => {
                if (result == true) {
                    axios
                        .put('/admin/invoice-scheme/' + self.invoiceSchemeId, data)
                        .then(function(response) {
                            self.dialog = false;
